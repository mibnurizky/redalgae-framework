...
class SSP {
    ...
    static function complex ( $request, $table, $primaryKey, $columns, $whereResult=null, $whereAll=null )
    {
        ...
        foreach ($columns as $col) {
            if (!empty($col['db'])) {
                if (strpos($col['db'], '.') !== false) {
                    list($tbl, $c) = explode('.', $col['db'], 2);
                    $column = "`$tbl`.`$c`";
                } else {
                    $column = "`".$col['db']."`";
                }
        ...
        if (strpos($primaryKey, '.') !== false) {
            list($tbl, $col) = explode('.', $primaryKey, 2);
            $sql = "SELECT COUNT(`$tbl`.`$col`) FROM $table $where";
        } else {
        ...
        if (strpos($primaryKey, '.') !== false) {
            list($tbl, $col) = explode('.', $primaryKey, 2);
            $sql = "SELECT COUNT(`$tbl`.`$col`) FROM $table $whereAllSql";
        } else {
